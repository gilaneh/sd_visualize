odoo.define('sd_visualize.VisualView', function (require) {
"use strict";

var BasicView = require('web.BasicView');
var Context = require('web.Context');
var core = require('web.core');
var VisualController = require('sd_visualize.VisualController');
var VisualRenderer = require('sd_visualize.VisualRenderer');
const { generateID } = require('web.utils');

var _lt = core._lt;

var VisualView = BasicView.extend({
    config: _.extend({}, BasicView.prototype.config, {
        Renderer: VisualRenderer,
        Controller: VisualController,
    }),
    display_name: _lt('Visual'),
    icon: 'fa-edit',
    multi_record: false,
    withSearchBar: false,
    searchMenuTypes: [],
    ...
    init: function (viewInfo, params) {
        ...
        this.rendererParams.isFromVisualViewDialog = params.isFromVisualViewDialog;
    ...
    _extractParamsFromAction: function (action) {
        ...
        if (inDialog || inline || fullscreen) {
            params.mode = 'edit';
        } else if (action.context && action.context.visual_view_initial_mode) {
            params.mode = action.context.visual_view_initial_mode;
        }
        return params;
    },
    /**
    ...
    _loadSubviews: function (parent) {
        ...
        if (this.loadParams && this.loadParams.fieldsInfo) {
            ...
            _.each(this.loadParams.fieldsInfo.visual, function (attrs, fieldName) {
                var field = fields[fieldName];
                if (!field) {
                    // when a one2many record is opened in a visual view, the fields
                    // of the main one2many view (list or kanban) are added to the
                    // fieldsInfo of its visual view, but those fields aren't in the
