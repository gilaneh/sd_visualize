odoo.define('sd_visualize.VisualController', function (require) {
"use strict";
...
var VisualController = BasicController.extend({
    custom_events: _.extend({}, BasicController.prototype.custom_events, {
        ...
        visual_dialog_discarded: '_onVisualDialogDiscarded',
    ...
    /**
     * This hook is called when a visual view is restored (by clicking on the
     * breadcrumbs). In general, we force mode back to readonly, because
     * whenever we leave a visual view by stacking another action on the top of
        ...
        if (!this.disableAutofocus) {
            ...
            if (!isControlActivted) {
                ...
                if (this.$buttons && this.mode === 'readonly') {
                    return this.$buttons.find('.o_visual_button_edit').focus();
                }
            }
        }
    },
    ...
    /**
     * This method switches the visual view in edit mode, with a new record.
     *
    ...
    /**
     * Render buttons for the control panel.  The visual view can be rendered in
    ...
    renderButtons: function ($node) {
        ...
        if ((this.defaultButtons && !this.$buttons) || mustRenderFooterButtons) {
            ...
            if (mustRenderFooterButtons) {
                ...
                this.$buttons.append(qweb.render("VisualView.buttons", {widget: this}));
                this.$buttons.on('click', '.o_visual_button_edit', this._onEdit.bind(this));
                this.$buttons.on('click', '.o_visual_button_create', this._onCreate.bind(this));
                this.$buttons.on('click', '.o_visual_button_save', this._onSave.bind(this));
                this.$buttons.on('click', '.o_visual_button_cancel', this._onDiscard.bind(this));
                this._assignSaveCancelKeyboardBehavior(this.$buttons.find('.o_visual_buttons_edit'));
                this.$buttons.find('.o_visual_buttons_edit').tooltip({
    ...
    /**
     * The visual view has to prevent a click on the pager if the visual is dirty
     *
    ...
    /**
     * Overrides to force the viewType to 'visual', so that we ensure that the
     * correct fields are reloaded (this is only useful for one2many visual views).
     *
     * @override
     */
    update: async function (params, options) {
        ...
        params = _.extend({viewType: 'visual', mode: this.mode}, params);
    ...
    updateButtons: function () {
        ...
        this.$buttons.find('.o_visual_buttons_edit')
            .toggleClass('o_hidden', !edit_mode);
        this.$buttons.find('.o_visual_buttons_view')
            .toggleClass('o_hidden', edit_mode);
    },
    ...
    /**
     * Overrides to reload the visual when saving failed in readonly (e.g. after
    ...
    _onButtonClicked: function (ev) {
        // stop the event's propagation as a visual controller might have other
        // visual controllers in its descendants (e.g. in a VisualViewDialog)
        ev.stopPropagation();
        var self = this;
        var def;
        ...
        // Kind of hack for VisualViewDialog: button on footer should trigger the dialog closing
     ...
     * @param {OdooEvent} event
     */
    _onVisualDialogDiscarded: function(ev) {
        ev.stopPropagation();
     ...
     * but later on when he clicks on 'Save' in the main visual view. For this to
     ...
     * with the one of the visual view.
     *
     * @private
     * @param {OdooEvent} ev
     */
    _onOpenOne2ManyRecord: async function (ev) {
        ...
        new dialogs.VisualViewDialog(this, {
    ...
    /**
     * Open an existing record in a visual view dialog
     *
     * @private
     * @param {OdooEvent} ev
     */
    _onOpenRecord: function (ev) {
        ...
        new dialogs.VisualViewDialog(self, {
