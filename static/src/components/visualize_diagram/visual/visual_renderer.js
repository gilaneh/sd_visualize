odoo.define('sd_visualize.VisualRenderer', function (require) {
"use strict";
...
const symbol = Symbol('visual');

var VisualRenderer = BasicRenderer.extend({
    className: "o_visual_view",
    events: _.extend({}, BasicRenderer.prototype.events, {
        ...
        'click .o_visual_label': '_onFieldLabelClicked',
    }),
    ...
    init: function (parent, state, params) {
        ...
        // The visual renderer doesn't render invsible fields (invisible="1") by
     ...
     */
    start: function () {
        this._applyVisualSizeClass();
        return this._super.apply(this, arguments);
    },
    /**
     * Called each time the visual view is attached into the DOM
     */
    on_attach_callback: function () {
     ...
     /**
     * Called each time the visual view is detached into the DOM
     */
     ...
     * In read mode, delegate which button to give the focus to, to the visual_renderer
     *
    ...
    autofocus: function () {
        if (this.mode === 'readonly') {
            var firstPrimaryVisualButton =  this.$el.find('button.btn-primary:enabled:visible:first()');
            if (firstPrimaryVisualButton.length > 0) {
                return firstPrimaryVisualButton.focus();
            } else {
                return;
            }
        }
    ...
    displayTranslationAlert: function () {
        ...
        if (this.alertFields[this.state.res_id]) {
            ...
            if (this.$('.o_visual_statusbar').length) {
                this.$('.o_visual_statusbar').after($notification);
            } else if (this.$('.o_visual_sheet_bg').length) {
                this.$('.o_visual_sheet_bg').prepend($notification);
            } else {
     ...
     * visual that has opened it and ensures that the focus is in the correct
     * field.
     */
    ...
    getLocalState: function () {
        const state = {};
        const sheetBg = this.el.querySelector('.o_visual_sheet_bg');
        if (sheetBg) {
    ...
    /**
     * Resets state which stores invisualation like scroll position, curently
     * active page, ...
     *
     * @override
     */
    ...
    _activateNextFieldWidget: function (record, currentIndex) {
        //if we are the last widget, we should give the focus to the first Primary Button in the visual
    ...
    _applyVisualSizeClass: function () {
        const visualEl = this.$el[0];
        if (config.device.size_class <= config.device.SIZES.XS) {
            visualEl.classList.add('o_xxs_visual_view');
        } else {
            visualEl.classList.remove('o_xxs_visual_view');
        }
        if (config.device.size_class === config.device.SIZES.XXL) {
            visualEl.classList.add('o_xxl_visual_view');
        } else {
            visualEl.classList.remove('o_xxl_visual_view');
        }
    },
    /**
     * @private
    ...
    /**
     * This function is called once visual view is rendered or modifiers are
     * changed to process labels to add o_visual_label_empty.
     *
     * @private
     */
    _postProcessLabels() {
    ...
    */
     ...
     * @returns {jQueryElement}
     */
    _renderTagVisual: function (node) {
        var $result = $('<div/>');
        if (node.attrs.class) {
    ...
    _renderTagHeader: function (node) {
        var self = this;
        var $statusbar = $('<div>', {class: 'o_visual_statusbar'});
    ...
    _renderTagLabel: function (node) {
        ...
        if (fieldName) {
            modifiersOptions = {
                callback: function (element, modifiers, record) {
                    ...
                    element.$el.toggleClass('o_visual_label_empty', !!( // FIXME condition is evaluated twice (label AND widget...)
                        record.data.id
        ...
        // this worked in 10.0 for "o_visual_label_empty" reevaluation but not for
    ...
    _renderTagSheet: function (node) {
        this.has_sheet = true;
        var $sheet = $('<div>', {class: 'clearfix position-relative o_visual_sheet'});
    ...
    _renderView: function () {
        ...
        // render the visual and evaluate the modifiers
        var defs = [];
        this.defs = defs;
        this.inactiveNotebooks = [];
        var $visual = this._renderNode(this.arch).addClass(this.className);
        delete this.defs;

        return Promise.all(defs).then(() => this.__renderView()).then(function () {
            self._postProcessLabels();
            self._updateView($visual.contents());
    ...
    /**
     * Updates the visual's $el with new content.
     *
     * @private
     * @see _renderView
    ...
    _updateView: function ($newContent) {
        ...
        // Set the new content of the visual view, and toggle classnames
        this.$el.html($newContent);
        this.$el.toggleClass('o_visual_nosheet', !this.has_sheet);
        if (this.has_sheet) {
            this.$el.children().not('.o_VisualRenderer_chatterContainer')
                .wrapAll($('<div/>', {class: 'o_visual_sheet_bg', 'data-command-category': "app"}));
        }
        this.$el.toggleClass('o_visual_editable', this.mode === 'edit');
        this.$el.toggleClass('o_visual_readonly', this.mode === 'readonly');
        ...
        _.each(this.allFieldWidgets[this.state.id], function (widget) {
            ...
            var $label = idForLabel ? self.$('.o_visual_label[for=' + idForLabel + ']') : $();
